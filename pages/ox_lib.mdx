...
<ResourceLinks repo="https://github.com/opensourcewizards/ox_lib" />
...
### Download a [release](https://github.com/opensourcewizards/ox_lib/releases) or build the source code.

```bash
git clone https://github.com/opensourcewizards/ox_lib.git
cd ox_lib/web
pnpm i
pnpm build
```
...
You can get the npm package [here](https://npmjs.com/package/@opensourcewizards/ox_lib).
...
```ts
import lib from '@opensourcewizards/ox_lib/server';
```

```ts
import { versionCheck } from '@opensourcewizards/ox_lib/server';
```
